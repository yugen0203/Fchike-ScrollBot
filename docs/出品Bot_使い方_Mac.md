...
1. 配布されたダウンロードリンクを開きます。
   - 出品Bot（Mac版）: **`出品Bot.app`**（または `ListBot` フォルダ）
   - ※Windows版（`ListBot-Windows.zip`）はMacでは動きません。Mac用を使ってください。
2. ダウンロードした **`出品Bot.app`** を、わかりやすい場所（例：`アプリケーション` フォルダや `書類`）に置きます。

---

## 3. はじめての起動（Macのセキュリティの通し方）

Macは、配布元が未登録のアプリを初回はブロックします。**初回だけ**次のやり方で開きます。

1. **`出品Bot.app`** を **右クリック（または Control＋クリック）** します。
2. 出たメニューから **「開く」** を選びます。
3. 「開いてもよろしいですか？」の確認が出たら、もう一度 **「開く」** を押します。

> 💡 2回目以降は普通にダブルクリックで開けます。
> 💡 「壊れている」「開けません」と出る場合 → 第9章を見てください。

しばらくすると、下のような **「出品Bot」** の画面が開きます。
...
> 🔐 入力した情報はあなたのMacの中だけに保存されます（次回からは入力不要）。
> 保存先：`~/Library/Application Support/ListBot/`
...
| こんなとき | どうする |
|---|---|
| 「開発元を確認できないため開けません」 | **右クリック →「開く」** で開く（第3章）。 |
| 「"出品Bot"は壊れているため開けません」と出る | アプリのコピー時に隔離属性が付いた可能性。ターミナルで `xattr -dr com.apple.quarantine "/アプリの場所/出品Bot.app"` を実行してから開いてください（場所はappをターミナルにドラッグすると入力できます）。 |
...
**Q. アプリを閉じてもいい？**
A. 出品処理が**終わってから**閉じてください。処理後にブラウザだけ残るのは正常です（アプリを閉じてもブラウザは残ります）。

**Q. 本当に出品されてしまうのが不安です。**
A. **確認モード**をオンにすれば、最後の「出品する」を自分で押すまで確定しません。

**Q. ログイン情報はどこに保存されますか？**
A. あなたのMacの中（`~/Library/Application Support/ListBot/`）だけです。インターネットには送られません。
